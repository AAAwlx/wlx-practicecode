...
void thread_recv(const std::string &ID, int cfd, const std::string &chatobject)
{
    while (1)
    {
        ...
        if (r.length() > 0)
        {
            ...
            {
                cout << "文件接收开始，实时通知将暂时阻塞，请在相关页面查看错过的消息" << endl;
                ...
                masqueue.pop();
                qmutex.unlock();
                cout << "文件接收结束，实时通知恢复" << endl;
            }
            else if (o == "sendfilep")
            { 
                string id = m.Deserialization("ID");
                string s = "id为" + id + "的好友请求向你传输一份文件";
                Clenit::PrientfT(s);
            }else if (o == "sendfileg")
            {
                string id = m.Deserialization("ID");
                string groupid = m.Deserialization("groupid");
                string s = "id为" + id + "的成员请求向群号为"+groupid+"传输了一份文件";
                Clenit::PrientfT(s);
            }
            else
            { // 非实时消息
